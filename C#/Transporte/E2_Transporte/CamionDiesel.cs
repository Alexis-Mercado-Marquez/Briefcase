...
namespace E2_Transporte
{
    class CamionDiesel : Camiones
    {
        ...
        public CamionDiesel(string color, string chofer, float tarifa, float tarifa_est, string pago, string placa, int modelo, int capacidad, float velocMax, float capacLitros, int numCilindros, bool bio) : 
        ...
        public override void Insertar_en_BD()
        {
            try //Por si errores
            {
                ...
                {   //Y se inserta el registro de su motor
                    ...
                    conexion.Close();
                    MessageBox.Show("Ha insertado exitosamente el registro del camión a diesel");
                }
            }
            catch (Exception ex)
