...
{
    class CamionHidrogeno : Camiones
    {
        float velocMax;
        int por_Eficiencia;
        ...
        public CamionHidrogeno(string color, string chofer, float tarifa, float tarifa_est, string pago, string placa, int modelo, int capacidad, float velocMax, int por_Eficiencia) : 
        ...
        public override void Insertar_en_BD()
        {
            try //Por si errores
            {
                ...
                {   //Y se inserta el registro de su motor
                    ...
                    conexion.Close();
                    MessageBox.Show("Ha insertado exitosamente el registro del camión de hidrogeno");
                }
            }
            catch (Exception ex)
