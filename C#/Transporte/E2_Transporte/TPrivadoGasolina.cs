...
{
    class TPrivadoGasolina : Transporte_Priv
    {
        float velocMax;
        float capacLitros;
        int numCilindros;
        ...
        public TPrivadoGasolina(string color, string chofer, float tarifa, float costo_min, string placa, int modelo, int capacidad, float velocMax, float capacLitros, int numCilindros) : 
        ...
        public override void Insertar_en_BD()
        {
            try //Por si errores
            {
                ...
                {   //Y se inserta el registro de su motor
                    ...
                    conexion.Close();
                    MessageBox.Show("Ha insertado exitosamente el registro del transporte privado a gasolina");
                }
            }
            catch (Exception ex)
