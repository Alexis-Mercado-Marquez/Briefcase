...
{
    class TPrivadoHidrogeno : Transporte_Priv
    {
        float velocMax;
        int por_Eficiencia;
        ...
        public TPrivadoHidrogeno(string color, string chofer, float tarifa, float costo_min, string placa, int modelo, int capacidad, float velocMax, int por_Eficiencia) : 
        ...
        public override void Insertar_en_BD()
        {
            try //Por si errores
            {
                ...
                {   //Y se inserta el registro de su motor
                    ...
                    conexion.Close();
                    MessageBox.Show("Ha insertado exitosamente el registro del transporte privado de hidrogeno");
                }
            }
            catch (Exception ex)
