...
{
    class TPrivadoMixto : Transporte_Priv 
    {
        float velocMax;
        float capacLitros;
        int numCilindros;
        ...
        public TPrivadoMixto(string color, string chofer, float tarifa, float costo_min, string placa, int modelo, int capacidad, float velocMax, float capacLitros, int numCilindros, float cargaMax) : 
        ...
        public override void Insertar_en_BD()
        {
            try //Por si errores
            {
                ...
                {   //Y se inserta el registro de su motor
                    ...
                    conexion.Close();
                    MessageBox.Show("Ha insertado exitosamente el transporte privado hibrido");
                }
            }
            catch (Exception ex)
