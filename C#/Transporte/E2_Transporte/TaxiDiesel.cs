...
{
    class TaxiDiesel : Taxis_de_Ruta
    {
        float velocMax;
        float capacLitros;
        int numCilindros;
        ...
        public TaxiDiesel(string color, string chofer, float tarifa, float costo_min, string placa, int modelo, float velocMax, float capacLitros, int numCilindros, bool bio) : 
        ...
        public override void Insertar_en_BD()
        {
            try //Por si errores
            {
                ...
                {   //Y se inserta el registro de su motor
                    ...
                    conexion.Close();
                    MessageBox.Show("Ha insertado exitosamente el registro del taxi a diesel");
                }
            }
            catch (Exception ex)
