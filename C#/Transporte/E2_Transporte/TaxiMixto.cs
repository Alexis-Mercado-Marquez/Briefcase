...
namespace E2_Transporte
{
    class TaxiMixto : Taxis_de_Ruta
    {
        ...
        public TaxiMixto(string color, string chofer, float tarifa, float costo_min, string placa, int modelo, float velocMax, float capacLitros, int numCilindros, float cargaMax) : 
        ...
        public override void Insertar_en_BD()
        {
            try //Por si errores
            {
                ...
                {   //Y se inserta el registro de su motor
                    ...
                    conexion.Close();
                    MessageBox.Show("Ha insertado exitosamente el taxi hibrido");
                }
            }
            catch (Exception ex)
